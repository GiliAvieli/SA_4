...
-- Stored Procedure: sp_process_warehouse_operation (FINAL VERSION)
...
-- FIX: Use correct column name 'issuedToId' (INT, FK to requester)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @CurrentStatus = reservationStatus, @RequestedById = requestedById FROM EquipmentReservation
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    -- issuedToId = requestedById from the reservation (who requested it)
                    -- condition defaults to 'unknown' until item is returned
                    INSERT INTO EquipmentIssue (equipmentissue_id, equipmentId, reservationId, reservation_detail_id, issued_qty, returned_ok_qty, lost_qty, damaged_qty, status, issueDate, issuedToId, condition)
                    VALUES (@NextIssueId, @EquipmentId, @equipmentreservation_id, @ReservationDetailId, @DeliveredQty, 0, 0, 0, 'Active', GETDATE(), @RequestedById, 'unknown');
        ...
        -- =====================================================================
        -- AUTO-ARCHIVE: Only close if RETURN phase is complete
        -- =====================================================================
        ...
        -- Count items with completed returns (status = 'Completed' means return was recorded)
        ...
        -- Only close reservation if ALL items have been returned/accounted for
        -- This means: issued_qty = returned_ok_qty + lost_qty + damaged_qty for ALL items
...
PRINT 'sp_process_warehouse_operation FINAL created successfully!';
